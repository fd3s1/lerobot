...
void HumanInfo::YoloHumanDetectCallback(const object_detection_msgs::RealsenseTargets::ConstPtr& _msg)
{
    ...
    this->human_status.update = (_msg->realsense_targets.size() != 0);
...
{
    this->yolo_human_pub = this->nh.advertise<object_detection_msgs::TargetStatus>("/drone0/detection_status/human", 5);
    this->yolo_human_sub = this->nh.subscribe<object_detection_msgs::RealsenseTargets>("/drone0/yolov8_detector/realsense_targets",
                                                                                1,
