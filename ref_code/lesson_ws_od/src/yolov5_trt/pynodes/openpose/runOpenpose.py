...
def infer_fast(net, img, net_input_height_size, stride, upsample_ratio, cpu,
    ...
    # if not cpu:         ##########changed by cl 2023_04_15
    ...
    for img in image_provider:  # 遍历图像集
        ...
        if i % 1 == 0:
            ...
            pose_entries, all_keypoints = group_keypoints(all_keypoints_by_type, pafs,
                                                          demo=True)  # 得到所有分配的人（前18维为每个人各个关节点在所有关节点中的索引，后两维为每个人得分及每个人关节点数量），及所有关节点信息
            ...
            for n in range(len(pose_entries)):  # 依次遍历找到的每个人
                ...
                if boxList:
                    ...
                    print(posebox)
                    human_pose_box = posebox
                    # [(posebox[0] + posebox[2] / 2),( posebox[1] + posebox[3] / 2)]
                    print('coincideValue:' + str(coincideValue))
                    if coincideValue < 0.2:
                        normalFlag = False
            ...
            # 保存识别后的照片
            # cv2.imwrite(f'C:/zqr/project/yolov5_openpose/Image/{t}.jpg', img)
            # cv2.imshow('Lightweight Human Pose Estimation Python Demo', img)  #取消显示

            cv2.waitKey(1)
        i += 1
    # cv2.destroyAllWindows()  #取消摧毁
...
def detect_main(video_name=''):
    ...
    net = jit.load(r'./openpose.jit')

    # *************************************************************************
    action_net = jit.load(r'./action.jit')
    # ************************************************************************
