...
void predict_state_callback(const ros::TimerEvent& event) {
  ...
  if (update_dt < 2.0) {
    ekfPtr_->predict();//更新 预测值
  } else {
    ROS_WARN("[ekf] too long time no update!");
    return;
  }
  // publish target odom 发布目标odom 
  ...
  target_odom_pub_.publish(target_odom);//发布target_ekf_odom话题(在simulation1.launch中remap了话题名)
}
...
void update_state_callback(const nav_msgs::OdometryConstPtr& target_msg, const nav_msgs::OdometryConstPtr& odom_msg) {
  ...
  Eigen::Vector3d cam_p = odom_q.toRotationMatrix() * cam2body_p_ + odom_p;//从相机坐标系转移到机体坐标系
  Eigen::Quaterniond cam_q = odom_q * Eigen::Quaterniond(cam2body_R_);

  p.x() = target_msg->pose.pose.position.x;//p中存放目标无人机当前位置
  ...
  Eigen::Vector3d rpy = quaternion2euler(q);//rpy存放目标无人机当前欧拉角

  // NOTE check whether it's in FOV  检查是否在FOV范围内
  if (check_fov_) {
  ...
  // update target odom 更新目标无人机的odom
  double update_dt = (ros::Time::now() - last_update_stamp_).toSec();
  if (update_dt > 5.0) {
    ekfPtr_->reset(p, rpy);//更新目标位置和欧拉角
    ROS_WARN("[ekf] reset!");
...
int main(int argc, char** argv) {
  ...
  nh.getParam("cam_fx", fx_);//346.74048
  nh.getParam("cam_fy", fy_);//349.12708
  nh.getParam("cam_cx", cx_);//322.83899
  nh.getParam("cam_cy", cy_);//234.54992
  nh.getParam("cam_width", width_);//640
  nh.getParam("cam_height", height_);//480
  nh.getParam("pitch_thr", pitch_thr_);//37
  nh.getParam("check_fov", check_fov_);//false
  ...
  int ekf_rate = 20;//每秒20次
  nh.getParam("ekf_rate", ekf_rate);
  ekfPtr_ = std::make_shared<Ekf>(1.0 / ekf_rate);

  yolo_sub_.subscribe(nh, "yolo", 1, ros::TransportHints().tcpNoDelay());//订阅/target/odom话题
  odom_sub_.subscribe(nh, "odom", 100, ros::TransportHints().tcpNoDelay());//订阅/drone0/odom话题
