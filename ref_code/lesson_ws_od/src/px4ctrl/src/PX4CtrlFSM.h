...
public:
	...
	// inline geometry_msgs::PoseStamped vision_pose_cb(const geometry_msgs::PoseStamped::ConstPtr& msg)
	// {
	// 	geometry_msgs::PoseStamped local_msg;
	// 	local_msg.header=msg->header;
	// 	local_msg.pose= msg->pose;
	// 	std::cout << "local_msg=" << local_msg.pose.position.x << std::endl;
	// 	return local_msg;
	// }
	

	//LinearControl &controller;
	ros::Subscriber manual_flag_sub;
	...
	void manual_flag_cb(const std_msgs::UInt8 msg);
...
private:
	...
	void publish_bodyrate_ctrl(const Desired_State_t &des, const ros::Time &stamp);
	void publish_attitude_ctrl(const Desired_State_t &des, const ros::Time &stamp);
	void publish_position_ctrl(const Desired_State_t &des, const ros::Time &stamp);
	void publish_trigger(const geometry_msgs::PoseStamped &odom_msg);
