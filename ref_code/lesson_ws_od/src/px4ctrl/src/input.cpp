...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg)
{
    ...
    mode = ((double)msg.channels[4] - 1000.0) / 1000.0;//5通
    gear = ((double)msg.channels[5] - 1000.0) / 1000.0;//6通
    reboot_cmd = ((double)msg.channels[7] - 1000.0) / 1000.0;//8通
...
void Odom_Data_t::feed(geometry_msgs::PoseStampedConstPtr pMsg)
{
    ...
    uav_utils::extract_odometry(pMsg, p ,q);
...
// #define VEL_IN_BODY
    ...
    if ( (now - last_clear_count_time).toSec() > 1.0 )
    {
        if ( one_min_count < 25 )
        {
            ROS_WARN("ODOM frequency seems lower than 5Hz, which is too low!");
        }
        one_min_count = 0;
...
void Command_Data_t::feed(geometry_msgs::PoseStampedConstPtr pMsg)
{
    ...
    msg.pose=pMsg->pose;
     //std::cout << "cmd_sub=" << msg.pose.position.x << std::endl;
