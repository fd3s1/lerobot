...
echo "[auto-record-grasp-place] soft grasp: step=${GRASP_STEP_SIZE} settle=${GRASP_STEP_SETTLE_S}s min=${GRASP_CLOSE_MIN} current_delta=${GRASP_CONTACT_CURRENT_DELTA} load_delta=${GRASP_CONTACT_LOAD_DELTA} pos_err=${GRASP_POSITION_ERROR_THRESHOLD} angle_lag=${GRASP_ANGLE_CONTACT_DELTA} stall=${GRASP_STALL_DELTA} min_close=${GRASP_CONTACT_MIN_CLOSE_DELTA}"
