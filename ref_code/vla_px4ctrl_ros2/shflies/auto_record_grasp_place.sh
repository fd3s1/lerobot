...
ATTITUDE_SOFT_MODE_TOPIC="${ATTITUDE_SOFT_MODE_TOPIC:-/px4ctrl/attitude_soft_mode}"
...
GRASP_COMPLIANCE_RADIUS_M="${GRASP_COMPLIANCE_RADIUS_M:-0.35}"
GRASP_COMPLIANCE_ELASTIC_GAIN="${GRASP_COMPLIANCE_ELASTIC_GAIN:-0.15}"
GRASP_ABORT_DRIFT_M="${GRASP_ABORT_DRIFT_M:-0.80}"
GRASP_ATTITUDE_SOFT_RAD="${GRASP_ATTITUDE_SOFT_RAD:-0.30}"
GRASP_ABORT_ATTITUDE_RAD="${GRASP_ABORT_ATTITUDE_RAD:-0.65}"
...
echo "[auto-record-grasp-place] attitude soft mode topic: ${ATTITUDE_SOFT_MODE_TOPIC}"
...
echo "[auto-record-grasp-place] soft grasp balance/compliance: load_diff=${GRASP_BALANCE_LOAD_DIFF} angle_diff=${GRASP_ANGLE_BALANCE_DIFF} balance_step=${GRASP_BALANCE_STEP} max_balance=${GRASP_MAX_BALANCE_STEPS} compliance_radius=${GRASP_COMPLIANCE_RADIUS_M} elastic_gain=${GRASP_COMPLIANCE_ELASTIC_GAIN} abort_drift=${GRASP_ABORT_DRIFT_M} attitude_soft=${GRASP_ATTITUDE_SOFT_RAD} attitude_abort=${GRASP_ABORT_ATTITUDE_RAD}"
...
auto_args=(
  ...
  --attitude-soft-mode-topic "${ATTITUDE_SOFT_MODE_TOPIC}"
  ...
  --grasp-compliance-elastic-gain "${GRASP_COMPLIANCE_ELASTIC_GAIN}"
  --grasp-abort-drift-m "${GRASP_ABORT_DRIFT_M}"
  --grasp-attitude-soft-rad "${GRASP_ATTITUDE_SOFT_RAD}"
  --grasp-abort-attitude-rad "${GRASP_ABORT_ATTITUDE_RAD}"
)
