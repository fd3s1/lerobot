...
echo "[auto-hls-grasp-place] rc safety: topic=${RC_TOPIC} timeout=${RC_TIMEOUT_S}s stale_action=${RC_STALE_ACTION} ch10_index=${CH10_INDEX} open<=${CH10_OPEN_PWM} close>=${CH10_CLOSE_PWM} force_open_below_z=${FORCE_OPEN_BELOW_Z} mavros_state=${MAVROS_STATE_TOPIC}"
...
echo "[auto-hls-grasp-place] centering: kp=${CENTER_KP} vmax=${CENTER_VMAX_MPS} deadband=${CENTER_DEADBAND_M} offset_max=${CENTER_OFFSET_MAX_M} sign=${CENTER_COMMAND_SIGN} hls_error_gain=${HLS_CENTER_ERROR_GAIN} snapshot_mismatch_tol=${HLS_STATUS_CENTER_MISMATCH_TOL_M}"
