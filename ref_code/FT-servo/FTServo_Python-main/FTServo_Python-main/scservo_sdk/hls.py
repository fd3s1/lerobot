...
class hls(protocol_packet_handler):
    ...
    def ReadLoad(self, scs_id):
        scs_present_load, scs_comm_result, scs_error = self.read2ByteTxRx(scs_id, HLS_PRESENT_LOAD_L)
        return self.scs_tohost(scs_present_load, 10), scs_comm_result, scs_error

    def ReadVoltage(self, scs_id):
        return self.read1ByteTxRx(scs_id, HLS_PRESENT_VOLTAGE)

    def ReadTemper(self, scs_id):
        return self.read1ByteTxRx(scs_id, HLS_PRESENT_TEMPERATURE)

    def ReadCurrent(self, scs_id):
        scs_present_current, scs_comm_result, scs_error = self.read2ByteTxRx(scs_id, HLS_PRESENT_CURRENT_L)
    ...
    def ServoMode(self, scs_id):
        return self.write1ByteTxRx(scs_id, HLS_MODE, 0)

    def EleMode(self, scs_id):
    ...
    def WriteEle(self, scs_id, torque):
        torque = self.scs_toscs(torque, 15)
        return self.write2ByteTxRx(scs_id, HLS_GOAL_TORQUE_L, torque)

    def EnableTorque(self, scs_id, enable):
        return self.write1ByteTxRx(scs_id, HLS_TORQUE_ENABLE, enable)

    def WriteTorqueLimit(self, scs_id, torque):
        return self.write2ByteTxRx(scs_id, HLS_TORQUE_LIMIT_L, torque)
